...
def floor_div(args):
    ...
    """
    "*** YOUR CODE HERE ***"
    dividend = args.first  # dividend = calc_eval(args.first) if isinstance(args.first, Pair) else: args.first
    divisor = args.rest  # divisor = calc_eval(args.rest.first) if isinstance(args.rest.first, Pair) else: args.rest.first
    if isinstance(divisor.first, Pair):
        return floor_div(Pair(dividend, Pair(calc_eval(divisor.first), divisor.rest)))
    elif divisor.rest is nil:
        return dividend // divisor.first
    else:
        return floor_div(Pair(dividend // divisor.first, divisor.rest))
...
def eval_define(expressions):
    ...
    values = calc_eval(expressions.rest.first)
    bindings[key] = values
    return key
